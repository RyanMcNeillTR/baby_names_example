---
title: "The Lincoln Effect"
output: rmarkdown::github_document
---

Thanks to some prodding from all-around good guy Ben Welsh at the LA Times, I decided to take a look at Oklahoma boys who are named Lincoln in light of L'Affair Riley at OU. 
...
You can [find the baby names here](https://catalog.data.gov/dataset/baby-names-from-social-security-card-applications-state-and-district-of-columbia-data). It's Social Security Administration data based on applications.

One limitation to this data is that it only includes names where n>5. 

Below is the code to download it, but I've commented it out because the data is in the Github repo. 
...
OK. So we have each state's data in a separate file. That's unfortunate. We'll use the FS package to cycle through each of them and import into a single table. 
...
OK, that's interesting. But we really need to compare to other states. And to do that, we need to control for the number of births in each state.

To do that, we need the CDC's count of births by state by year. Unfortunately, I don't think you can get state-level births via an API, so I downloaded the data [from CDC Wonder](https://wonder.cdc.gov/). 
...
natality_07_20 <- read_tsv("./data/natality_07_20.txt") %>%
  janitor::clean_names() 
```
I noticed they slyly included the totals for each year in the files. Let's toss em. And trim column names. 
...
Let's use [this little package](https://github.com/OpenIntroStat/usdata) to add abbreviation names. 
...
OK, now let's tinker with ggplot2 and check out Oklahoma's rate over time. 
...
But like we discussed earlier, we really need to compare against the national rate minus Oklahoma. This little chunk of code calculates that and stores it in an object for later use in ggplot. 
...
Ok, let's make a plot. I was tinkering with annotations, but as you'll see below, this method sucks.
...
Before moving on, let's also store Oklahoma's calculations in its own object for cleaner ggplot code. 
...
Let's put it all together. I'm too lazy to tinker with the colors too much.
